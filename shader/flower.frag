...
void main() {
   vec2 uv = vUv;
   float noise = pnoise(vec3(.5, vPosition.x, vPosition.y));
   vec3 color = vec3(vNoise * uForce, .1, vNoise).rgb;

   float mask = step(0., vPosition.x);
   if(mask == 0. ) {
      discard;
   } 
   vec3 finalColor = vec3(color * mask );

   gl_FragColor = vec4(finalColor, uTransparence * vProgress);
}
